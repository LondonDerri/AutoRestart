    ...
    private void tryCountdown(MinecraftServer server, long ms) {
        if (ms > countdown) {
            if (ms - lastCountdownBroadcast >= 1000) {
                server.getPlayerManager().broadcast(Text.literal(String.format(countdownMessage, (restartTime - ms) / 1000).replace(String.format(countdownMessage, 0), disconnectMessage)).formatted(Formatting.YELLOW), false);
